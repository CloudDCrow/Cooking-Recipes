    ...
    if (user) {
      ...
      */
      if (window.location.pathname.includes ('/view.html')) {    
        db.collection('users').doc(userUid).collection('recipes').get()
          .then(querySnapshot => {
            const recipeNames = [];
      ...
      if (recipeID) {
        db.collection('users').doc(userUid).collection('recipes').doc(recipeID).get()
          .then(doc => {
            if (doc.exists) {
              const recipeData = doc.data();
      ...
      if(submitButton) {
        submitButton.addEventListener('click', function (event) {
          ...
          db.collection('users').doc(userUid).collection('recipes').add(recipeData)
